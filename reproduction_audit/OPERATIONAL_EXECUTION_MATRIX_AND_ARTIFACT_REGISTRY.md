...
| [`15_verify_validation_atmospheric_pipeline.py`](../scripts/15_verify_validation_atmospheric_pipeline.py) | Validation Atmospheric Pipeline Verification Engine | Preflight engine verifying complete date coverage across all 210 validation cycles (2022–2023), 5-channel presence and ordering (`pwat, spfh, tmax, diff_temp, hgt_pres`), Candidate A grid conformity ($32 \times 48$), 126 active cell finiteness, and frozen normalization bounds alignment. Supports `--mode live` and `--mode mock`. | `[PASS / VERIFIED]` |
