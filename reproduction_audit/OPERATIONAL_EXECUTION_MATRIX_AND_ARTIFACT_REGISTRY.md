...
| **Phase 23** | Recursive Degradation Diagnostic [Track 1 Core Scientific Investigation] | Immediate post-A0 empirical diagnostic: error compounding across leads ($W_1 \to W_4$), Protocols 1–4 (standard recursion vs oracle truth vs direct non-recursive comparator vs error injection $D_k(\epsilon)$ with $[0,1]$ boundary clipping), computing primary recursive gap $\Delta E_k = E_k^{\text{recursive}} - E_k^{\text{oracle}}$ via case-level moving-block bootstrap (MBB with centered null $p_{\text{boot}} < 0.0001$) and supplementary paired Wilcoxon tests. Evaluated across 194 validation cases and 3 seeds on NVIDIA Tesla T4 in Notebook 13 (execution commit `b64970f`) | `[PASS / CERTIFIED_ON_GPU: 2026-09-17]` | Diagnostic Contract [`contracts/A0/PHASE_23_DIAGNOSTIC_CONTRACT.yaml`](../contracts/A0/PHASE_23_DIAGNOSTIC_CONTRACT.yaml); Engine `scripts/19_run_recursive_degradation_diagnostic.py`; Notebook `notebooks/13_mindanao_recursive_degradation_diagnostic.ipynb`; Results `logs/phase23_recursive_diagnostic_results.json`; Composite `figures/phase23_recursive_degradation_composite.png` |
| **Post-A0 Gate 2 (G2-R)** | Recursive Refinement Decision Gate [Track 1 Scientific Gate] | Formal decision gate evaluating statistical significance ($p_{\text{boot}} < 0.0001$), non-trivial effect size ($d_z \ge 0.20$), 95% MBB CIs, replicate consistency, and magnitude monotonicity ($|\epsilon|$). All 4 criteria satisfied (W3 $d_z = 1.201$, relative gap represents 35.38% of W3 recursive MAE; W4 $d_z = 1.269$, relative gap represents 44.12% of W4 recursive MAE under oracle counterfactual; monotonic perturbation response with nonlinear propagation across leads). Formally evaluated to GO; Phase 24 authorized | `[PASS / VERDICT: GO: 2026-09-17]` | Formal Decision Dossier [`reproduction_audit/POST_A0_GATE2_DECISION_DOSSIER.md`](POST_A0_GATE2_DECISION_DOSSIER.md) |
| **Phase 24** | Model A1 (Lead-Aware Recursive Residual Refinement) [Track 1 CONDITIONAL] | **Activated by Post-A0 Gate 2 (G2-R) GO**. Preserves frozen Model A0 backbone while introducing shared lightweight, lead-conditioned convolutional residual refinement block $\tilde{y}_k = \hat{y}_k + \mathcal{R}_\theta(\hat{y}_k, \hat{y}_{k-1}, k)$ across $W_1 \to W_2, W_2 \to W_3, W_3 \to W_4$ to suppress error compounding | `[SPECIFICATION FROZEN - READY FOR test_18]` | Architecture Contract [`contracts/A1/PHASE_24_A1_ARCHITECTURE_CONTRACT.yaml`](../contracts/A1/PHASE_24_A1_ARCHITECTURE_CONTRACT.yaml) |
...
│   └── POST_A0_GATE2_DECISION_DOSSIER.md
├── scripts/
...
│   └── phase23_recursive_diagnostic_results.json
├── figures/
...
│   └── phase23_recursive_degradation_composite.png
├── pilot_raw/
...
| [`PHASE_24_A1_ARCHITECTURE_CONTRACT.yaml`](../contracts/A1/PHASE_24_A1_ARCHITECTURE_CONTRACT.yaml) | Phase 24 Model A1 Architecture & Pre-Analysis Specification Contract | Pre-registers and freezes all architectural constraints, 100% frozen A0 backbone invariant (0 weights retrained), shared single convolutional module $\mathcal{R}_\theta(\cdot, k)$ across $W_1 \to W_2, W_2 \to W_3, W_3 \to W_4$ transitions with explicit lead conditioning $k \in \{1, 2, 3\}$, $<50\text{k}$ parameter target, direct supervised residual learning ($r_k^* = y_k^{\text{true}} - \hat{y}_k$) with detached A0 input features, domain clipping $[0, 1]$, and TensorFlow 2.x standardization. | `[PRE-ANALYSIS FROZEN]` |
...
| [`13_mindanao_recursive_degradation_diagnostic.ipynb`](../notebooks/13_mindanao_recursive_degradation_diagnostic.ipynb) | Phase 23 Recursive Degradation Diagnostic & Post-A0 Gate 2 (G2-R) Decision | 21 cells. Multi-protocol evaluation (Protocols 1, 2, 4) across 194 validation cases, primary Moving-Block Bootstrap significance ($p_{\text{boot}}$), supplementary Wilcoxon test, Holm-Bonferroni correction, Post-A0 Gate 2 (G2-R) unanimous GO evaluation, and verified Executive Findings & Scientific Certification Summary. | `[PASS / CERTIFIED_ON_GPU: 2026-09-17]` |
...
| [`POST_A0_GATE2_DECISION_DOSSIER.md`](POST_A0_GATE2_DECISION_DOSSIER.md) | Post-A0 Gate 2 (G2-R) | Formal decision dossier evaluating pre-registered GO/NO-GO criteria across 194 validation cases and 3 seeds on Tesla T4 GPU. Cleared with unanimous GO verdict; authorizes Phase 24. | `[PASS / VERDICT: GO: 2026-09-17]` |
| [`THESIS_RESEARCH_GAP_EMPIRICAL_EVIDENCES.md`](THESIS_RESEARCH_GAP_EMPIRICAL_EVIDENCES.md) | Research Gap Synthesis | Master thesis research gap dossier: mathematical proofs, multi-seed error decomposition tables, Moving-Block Bootstrap CIs, Holm-Bonferroni correction, Cohen's $d_z > 1.20$, and Protocol 4 perturbation monotonicity across 126 active cells. Authoritative reference for thesis Chapters 1, 3, and 4. | `[FORMALLY PROVEN / SEALED RESEARCH GAP]` |
...
| [`figures/phase23_recursive_degradation_composite.png`](../figures/phase23_recursive_degradation_composite.png) | 4-panel recursive degradation diagnostic composite: lead-wise recursive vs. oracle MAE comparison, empirical error gap with 95% MBB CIs, Protocol 4 perturbation sensitivity divergence curves across leads, and evaluation cell clipping census | 300 DPI PNG (581 KB) |
