...
| [`11_mindanao_a0_production_smoke_preflight.ipynb`](../notebooks/11_mindanao_a0_production_smoke_preflight.ipynb) | Pre-Production Gate 3 Production Training Smoke Preflight Execution & Audit Record | 11 cells. Validates 5 core production stages: Stage A (4-lead real backward pass, unmasked vs 126-cell masked MAE loss, finite gradients $\|\nabla_\theta\| > 0$, $\|\Delta w\| > 0$), Stage B (recursive channel semantics, exact indices $W_2=11, W_3=[3,4], W_4=[3,4,5]$, permutation tamper detection), Stage D (checkpoint parity scoping, model-weight vs full training state, verified step-2 trajectory equality), and Stage E (authoritative execution of `scripts/14_run_a0_production_smoke_test.py --mode certify` and telemetry sync to GCS lake). | `[READY FOR EXECUTION]` |
