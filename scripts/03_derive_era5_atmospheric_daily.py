...
def main():
    ...
    candidate_singles = [
        Path("parent_study_ex29/Data/raw_downloads/ERA5_atmospheric/single/era5_single_levels_2015_01.nc"),
        Path("Data/raw_downloads/ERA5_atmospheric/single/era5_single_levels_2015_01.nc"),
        Path("parent_study_ex29/Data/raw_downloads/ERA5_atmospheric/single/era5_single_levels_2015.nc"),
        Path("Data/raw_downloads/ERA5_atmospheric/single/era5_single_levels_2015.nc"),
    ]
    candidate_pressures = [
        Path("parent_study_ex29/Data/raw_downloads/ERA5_atmospheric/pressure/era5_z200_2015_01.nc"),
        Path("Data/raw_downloads/ERA5_atmospheric/pressure/era5_z200_2015_01.nc"),
        Path("parent_study_ex29/Data/raw_downloads/ERA5_atmospheric/pressure/era5_z200_2015.nc"),
        Path("Data/raw_downloads/ERA5_atmospheric/pressure/era5_z200_2015.nc"),
    ]
    mask_path = Path("processed/grid/mindanao_eval_mask_025.nc")
    grid_path = Path("processed/grid/mindanao_025deg.nc")

    actual_single = next((p for p in candidate_singles if p.exists()), None)
    if actual_single is None:
        print(f"[ERROR] Single-level raw file not found: checked {candidate_singles}")
        sys.exit(1)

    actual_pressure = next((p for p in candidate_pressures if p.exists()), None)
    if actual_pressure is None:
        print(f"[ERROR] Pressure-level raw file not found: checked {candidate_pressures}")
        sys.exit(1)
