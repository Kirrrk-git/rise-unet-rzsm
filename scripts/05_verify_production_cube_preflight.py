#!/usr/bin/env python3
"""
scripts/05_verify_production_cube_preflight.py
-------------------------------------------
Deterministic Preflight Verification Gate for the 11-Year Production RZSM Data Cube.
...
def check_gcs_archive_inventory() -> Dict[str, Any]:
    ...
    try:
        ...
        main_files = [f for f in file_names if f.startswith("era5-land-20") and "-sm3-" not in f and "antecedent" not in f]
        ...
        # Fallback to gcloud storage CLI with a strict timeout to avoid hanging when offline
        try:
            res = subprocess.run(
            ...
            return {
                ...
                "message": "GCS API inspection requires active Google Cloud credentials in environment (offline skip available)"
            }


def run_full_preflight_gate() -> Dict[str, Any]:
    print("=" * 80)
    print("PRODUCTION CUBE VERIFICATION GATE: 11-POINT PREFLIGHT INTEGRITY CHECK")
    print("=" * 80)
