...
- [ ] **Step 21K.3-pre**: Genuine Production-Path Model A0 Training Smoke Test across all 4 Leads with Evaluation Domain Masking. **`[CERTIFICATION_REVOKED_PENDING_REVISION_AND_RERUN]`**
  * **Certification Audit Findings & Governance Reset**: Following audit of Notebook 11 execution logs, Gate 3 certification was revoked due to: (1) Stage E fallback-to-PASS anti-pattern upon engine exception, (2) omission of Stage C from authoritative execution, (3) reliance on synthetic `np.random.uniform` tensors instead of genuine CaseBuilder data, and (4) an unresolved Step-2 optimization trajectory discrepancy.
  * **Hardened 5-Stage Scope**:
    - **Stage A**: Ingest real representative pilot case (`CASE_20150115_W01.npz`), normalized via frozen contract (`contracts/A0/normalization_parameters.yaml`), and verify finite gradients and non-zero weight updates ($\|\Delta w\| > 0$) across all 4 leads.
    - **Stage B**: Run genuine forward model inference cascade ($W_1 \to \hat{y}_1 \to W_2 \to \hat{y}_2 \to W_3 \to \hat{y}_3 \to W_4$) on real data, verifying exact recursive channel placement and hard permutation tamper guard.
    - **Stage C**: Explicit production loss path demonstrating 3-head deep supervision, comparing unmasked full bounding-box loss with authoritative 126-cell masked loss.
    - **Stage D**: Strict checkpoint scoping proving model-weight parity ($< 10^{-6}$) and full training-state next-step trajectory roundtrip ($< 10^{-6}$) using direct variable assignment.
    - **Stage E**: Authoritative fail-closed certification on physical Tesla T4 GPU in Google Colab (zero fallback permitted).
- [ ] **Step 21K.3**: Train Model A0 across minimum three predeclared seeds (seeds 42, 123, 456). **`[NOT_YET_AUTHORIZED]`**
  * **Prerequisites Status**: (1) 2022–2023 atmospheric mirroring certified under Pre-Production Gate 1 [PASS / CERTIFIED: 2026-09-16], (2) 21J hardware profiling & VRAM benchmark certified on physical Tesla T4 under Pre-Production Gate 2 [PASS / CERTIFIED_ON_GPU: 2026-09-17], and (3) 21K.3-pre [ACTIVE PRE-REQUISITE / REVISION & RERUN PENDING]. Full 3-Seed Model A0 Production Training will be authorized only upon authentic fail-closed Gate 3 certification.
