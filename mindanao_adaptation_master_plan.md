...
- [x] **Step 21K.3-pre**: Genuine Production-Path Model A0 Training Smoke Test across all 4 Leads with Evaluation Domain Masking. **`[PASS / VERIFIED / ACCEPTED: 2026-09-17: CERTIFIED_ON_GPU]`**
  * **Stage A (4-Lead Real Backward Pass & Parameter Updates)**: Instantiated genuine `UNET_RZSM` for Leads 1, 2, 3, and 4. Fed production-shaped normalized tensors $(B=11, 32, 48, C_k)$ with lead channel counts $C \in \{11, 12, 5, 6\}$. Executed full `tf.GradientTape()` backpropagation using 3-head deep supervision loss, and applied Adam optimizer step verifying finite gradient norms and $\|\Delta w\| > 0$ across all 4 leads (`[PASS]`).
  * **Stage B (Recursive Channel Semantics & Ordering Invariant)**: Ingested normalized $\hat{y}_{W1} \in [0, 1]$ into $W_2$ at channel index 11; verified $\hat{y}_{W1}$ and $\hat{y}_{W2}$ enter $W_3$ at channel indices 3 and 4; verified $\hat{y}_{W1}, \hat{y}_{W2}, \hat{y}_{W3}$ enter $W_4$ at channel indices 3, 4, 5. Verified zero double-normalization. Permutation tamper test caught scrambled input ordering cleanly (`[PASS]`).
  * **Stage C (Production Loss Path with Downstream Masking)**: Applied authoritative 126-cell binary evaluation mask (`eval_mask`) downstream in loss computation. Unmasked and masked evaluation-domain MAEs verified across all 4 leads (`[PASS]`).
  * **Stage D (Checkpoint Parity Scoping & Next-Step Trajectory Roundtrip)**: Verified bit-for-bit model-weight parity ($0.00 \times 10^0$) and full training-state restoration with verified step-2 optimization trajectory parity (loss delta $0.00\text{e}+00$, weight delta $5.02 \times 10^{-7} < 10^{-6}$; `[PASS]`).
  * **Stage E (Fail-Closed Certification Gate & Telemetry Export)**: Executed on physical Tesla T4 GPU in Google Colab (notebook `notebooks/11_mindanao_a0_production_smoke_preflight.ipynb`). Preflight telemetry artifact exported and synchronized to `gs://rise-unet-rzsm/reproduction_audit/a0_production_smoke_test.json`.
- [ ] **Step 21K.3**: Train Model A0 across minimum three predeclared seeds (seeds 42, 123, 456). **`[AUTHORIZED: 2026-09-17]`**
  * **Prerequisites Cleared**: (1) 2022–2023 atmospheric mirroring (48 monthly files) certified under Pre-Production Gate 1 [PASS / CERTIFIED: 2026-09-16], (2) 21J hardware profiling & VRAM benchmark certified on physical Tesla T4 under Pre-Production Gate 2 [PASS / CERTIFIED_ON_GPU: 2026-09-17], and (3) 21K.3-pre certified on physical Tesla T4 under Pre-Production Gate 3 [PASS / CERTIFIED_ON_GPU: 2026-09-17]. Full 3-Seed Model A0 Production Training is formally authorized to execute.
